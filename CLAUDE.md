...
Claude operates as **task manager and reviewer** — sets tasks, reviews results, manages pipeline.
GLM (via `glm` subagent) operates as **executor** — writes code, creates artifacts, implements features.

Workflow:
1. Claude analyzes requirements and creates tasks
2. Claude delegates implementation to GLM subagents
3. GLM executes and returns results
4. Claude reviews results and iterates if needed
