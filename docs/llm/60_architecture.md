...
| Instagram photo/carousel/login-walled post | `instagram_repost.go` | decline note; never queued (no retry can produce a video) |
...
retry queue for failed **TikTok exports**, **Instagram exports** and
**summarize** calls (commits 81f45c9, 47cdd99; Instagram added
2026-09-24). Public `/flush` (30s cooldown) processes
...
minus the audio gate; a post that carries no video is reported once
and dropped), summarize -> `retrySummarize` (re-runs
