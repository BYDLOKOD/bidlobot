...
type Config struct {
	...
	// Instagram repost egress. Instagram is blocked outright on some ISPs
	// and many posts answer an anonymous client with a login wall, so the
	// yt-dlp download accepts an optional proxy and a Netscape-format
	// cookie jar. Both empty - the default - mean a direct, anonymous
	// request; the bot starts either way and a blocked egress only sends
	// Instagram links into the deferred queue.
...
func loadConfig() Config {
	...
	return Config{
		...
		InstagramProxy:   envOr("INSTAGRAM_PROXY", ""),
		InstagramCookies: envOr("INSTAGRAM_COOKIES", ""),
...
func (c Config) Validate() error {
	...
	// supplied bad value is an error. Without the check a typo would
	...
	if c.InstagramCookies != "" {
		info, err := os.Stat(c.InstagramCookies)
		switch {
		case err != nil:
...
// validateProxyURL accepts the schemes yt-dlp's --proxy understands. A
// proxy string without a host is always a typo: the download would fail
